...
export const GET = handle(async (_req: Request, ctx: RouteContext<"/api/shots/[id]">) => {
...
export const PATCH = handle(async (req: Request, ctx: RouteContext<"/api/shots/[id]">) => {
  const { id } = await ctx.params;
  return ok(await updateShot(id, await body(req)));
});

export const DELETE = handle(async (_req: Request, ctx: RouteContext<"/api/shots/[id]">) => {
  const { id } = await ctx.params;
  await deleteShot(id);
  return ok({ ok: true });
});
