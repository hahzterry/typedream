...
/** Serve files from a project's data dir with HTTP Range support (video seeking). */
export async function GET(req: Request, ctx: RouteContext<"/api/projects/[pid]/files/[...path]">) {
  const { pid, path: segs } = await ctx.params;
  ...
  const abs = path.resolve(root, ...segs.map(decodeURIComponent));
