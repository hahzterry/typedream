 ...
 */
export const POST = handle(async (req: Request, ctx: RouteContext<"/api/projects/[pid]/assets/[id]/refs">) => {
  ...
  if (ct.includes("multipart/form-data")) {
    ...
    for (const f of files) {
      ...
      refs.push(await addRefFromBuffer(pid, id, buf, path.extname(f.name) || ".png", { label: label ?? path.basename(f.name, path.extname(f.name)) }));
    }
    return ok(refs, { status: 201 });
  }
  const b = (await req.json()) as { path?: string; url?: string; label?: string; useInVideo?: boolean };
  if (b.path) return ok(await addRefFromPath(pid, id, b.path, { label: b.label, useInVideo: b.useInVideo }), { status: 201 });
  if (b.url) return ok(await addRefFromUrl(pid, id, b.url, { label: b.label, useInVideo: b.useInVideo }), { status: 201 });
