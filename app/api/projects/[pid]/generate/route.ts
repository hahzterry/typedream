 ...
 * Default: every shot with no live or finished take.
 */
export const POST = handle(async (req: Request, ctx: RouteContext<"/api/projects/[pid]/generate">) => {
  ...
  for (const s of shots) {
    try {
      const takes = await generateShot(pid, s.id, { count: b.count });
