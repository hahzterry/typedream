...
export const GET = handle(async (_req: Request, ctx: RouteContext<"/api/projects/[pid]/takes/[id]">) => {
  const { pid, id } = await ctx.params;
  const t = getDb(pid).takes.find((x) => x.id === id);
  if (!t) throw new HttpError(404, "take not found");
  return ok(t);
});

/** rating / notes, or { select: true } to make it the shot's chosen take */
export const PATCH = handle(async (req: Request, ctx: RouteContext<"/api/projects/[pid]/takes/[id]">) => {
  const { pid, id } = await ctx.params;
  const b = await body<{ rating?: number; notes?: string; select?: boolean }>(req);
  const take = await mutate(pid, (db) => {
...
export const DELETE = handle(async (_req: Request, ctx: RouteContext<"/api/projects/[pid]/takes/[id]">) => {
  const { pid, id } = await ctx.params;
  const t = getDb(pid).takes.find((x) => x.id === id);
  if (!t) throw new HttpError(404, "take not found");
  if (t.status === "queued" || t.status === "running") {
    await cancelTake(pid, id);
  ...
  if (t.videoFile) await fsp.rm(outputPath(pid, t.videoFile), { force: true });
