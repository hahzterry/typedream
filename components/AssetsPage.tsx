...
const KINDS: { kind: AssetKind; label: string; blurb: string }[] = [
  { kind: "character", label: "Characters", blurb: "Reference images + an appearance lock keep each character consistent across shots." },
  { kind: "location", label: "Locations", blurb: "Backgrounds and sets. Attach one to a scene to send it with every shot." },
  { kind: "prop", label: "Props", blurb: "Objects that must keep their exact design." },
  { kind: "style", label: "Style refs", blurb: "Art-style images. One can be attached globally in Settings." },
];
...
export function AssetsPage() {
  ...
  return (
    <div className="grid gap-9">
      <div>
        <h1 className="text-xl font-semibold">Characters & Assets</h1>
        <p className="text-fg-3 text-sm mt-0.5">Reference @tags in shot prompts. Each asset&apos;s description is sent with the shot as a consistency lock.</p>
      </div>
        ...
        return (
          <section key={kind} className="grid gap-3">
            <div className="flex items-center gap-3 border-b border-line pb-2">
              ...
              <span className="text-fg-3 text-xs hidden md:inline">{blurb}</span>
      ...
      <AssetEditor open={!!editing} onClose={() => setEditing(null)} asset={editing && typeof editing === "object" ? editing : undefined} kind={typeof editing === "string" ? editing : undefined} />
    </div>
  );
}

function AssetCard({ asset, onEdit }: { asset: Asset; onEdit: () => void }) {
  const { api, busy, state, refUrl } = useHarness();
  ...
  async function upload(files: FileList | null) {
    ...
    try {
      await api(`/assets/${asset.id}/refs`, { form });
    } catch (e) {
  ...
  return (
    <div className="card p-4 grid gap-3 content-start">
      ...
      {asset.description ? <p className="text-xs text-fg-2 line-clamp-4">{asset.description}</p> : <p className="text-xs text-warn">No description — add an appearance lock so the model keeps the design.</p>}

      <div className="grid grid-cols-3 gap-2">
        {asset.refs.map((r) => (
          <div key={r.id} className={`relative group rounded-lg overflow-hidden border border-line ${r.useInVideo ? "" : "opacity-40"}`}>
            ...
            <div className="absolute top-1 right-1 hidden group-hover:flex gap-1">
              <button className="bg-black/70 rounded px-1 text-[10px]" title={r.useInVideo ? "exclude from video refs" : "include in video refs"} onClick={() => api(`/assets/${asset.id}/refs/${r.id}`, { method: "PATCH", body: { useInVideo: !r.useInVideo } })}>
                {r.useInVideo ? "on" : "off"}
              </button>
              <button className="bg-black/70 rounded px-1 text-[10px] text-err" onClick={() => confirm("Delete this reference?") && api(`/assets/${asset.id}/refs/${r.id}`, { method: "DELETE" })}>
                ×
              </button>
            </div>
          </div>
        ))}
        <button className="aspect-square rounded-lg border border-dashed border-line-2 text-fg-3 hover:text-fg hover:border-fg-3 text-xs flex flex-col items-center justify-center gap-1" onClick={() => fileInput.current?.click()} disabled={busy}>
      ...
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        <span className="mono text-[11px] text-fg-3 ml-auto">
          {asset.refs.filter((r) => r.useInVideo).length}/{asset.refs.length} used in video
        </span>
      </div>
      <ErrorLine error={err} />
...
function GenerateRefModal({ asset, open, onClose }: { asset: Asset; open: boolean; onClose: () => void }) {
  ...
  async function go() {
    ...
    setRunning(true);
    try {
      await api(`/assets/${asset.id}/refs/generate`, { body: { prompt: prompt || undefined, n, width: w, height: h, label: label || undefined } });
      onClose();
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
    } finally {
    ...
    <Modal open={open} onClose={onClose} title={`Generate reference images for ${asset.name}`}>
      <div className="grid gap-3">
        ...
        <Field label="Extra guidance">
          <textarea rows={4} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={asset.kind === "character" ? "three-quarter view, slight smile, white background" : "wide establishing view at sunset"} className="!font-sans !text-sm" />
        </Field>
...
function AssetEditor({ open, onClose, asset, kind }: { open: boolean; onClose: () => void; asset?: Asset; kind?: AssetKind }) {
  ...
  const formKey = open ? (asset?.id ?? `new-${kind}`) : undefined;
  if (formKey !== key) {
    setKey(formKey);
    setName(asset?.name ?? "");
    setTag(asset?.tag ?? "");
  ...
  async function save() {
    ...
    try {
      ...
      const saved = asset ? await api<Asset>(`/assets/${asset.id}`, { method: "PATCH", body }) : await api<Asset>("/assets", { body });
      const cur = state?.project.styleAssetId;
      if (isStyle && cur !== saved.id) await api("", { method: "PATCH", body: { styleAssetId: saved.id } });
      if (!isStyle && cur === saved.id) await api("", { method: "PATCH", body: { styleAssetId: null } });
      onClose();
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
    }
    ...
    <Modal open={open} onClose={onClose} title={asset ? `Edit ${asset.name}` : `New ${k}`}>
      <div className="grid gap-3">
        <div className="grid grid-cols-[1fr_160px_140px] gap-3">
          <Field label="Name">
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Aki Tanaka" autoFocus />
          </Field>
        ...
        >
          <textarea rows={5} value={desc} onChange={(e) => setDesc(e.target.value)} className="!font-sans !text-sm" />
        </Field>
        <Field label="Director notes (voice, personality, arcs — not sent to model)">
          <textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} className="!font-sans !text-sm" />
        </Field>
        {k === "style" && (
